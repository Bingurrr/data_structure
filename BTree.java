...
public class BTree {
	...
	public void deleteBT(BTree t, int m, int oldKey) {
		...
		do{
			...
			int pos = 0;
			if(nodePos == 0) {
				if(parent.childs[1].numKey >= ((m-1)/2)+1) pos = 1;
			}
			else if(nodePos == parent.numKey) {
				if(parent.childs[nodePos-1].numKey >= ((m-1)/2)+1) pos = -1;
			}
			else if(parent.childs[nodePos-1].numKey>=((m-1)/2)+1 || parent.childs[nodePos+1].numKey>=((m-1)/2)+1) {
				if(parent.childs[nodePos-1].numKey>=parent.childs[nodePos+1].numKey) pos = -1;
				else pos =  1;
			}
			if(pos != 0){
				if(pos ==-1) {
	...
	public void merge(BTNode parent,BTNode tmp, int m, int nodePos) {
		...
		int mergesibling = 0;
		if(nodePos == 0 ) mergesibling = 1;
		else if (nodePos == parent.numKey) mergesibling = -1;
		else {
